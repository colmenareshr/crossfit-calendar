...
const eventStyleGetter = (event: Event) => {
  ...
  if (event.title.toLowerCase().includes('kettlebell') || 
  ...
  const handleCreateEvent = (eventData: {
    ...
    const newEvent: Event = {
      ...
      eventType: eventData.title.toLowerCase().includes('open gym') 
        ? 'open-gym' 
        : eventData.title.toLowerCase().includes('kettlebell') || eventData.title.toLowerCase().includes('strength')
          ? 'specialty'
          : 'crossfit'
    }
  ...
  const handleReserve = () => {
    if (selectedEvent) {
      const message = encodeURIComponent(
        `Hola, me gustaría reservar el entrenamiento "${selectedEvent.title}" con ${selectedEvent.instructor} que comienza el ${moment(selectedEvent.start).format('DD/MM/YYYY HH:mm')}.`
      )
